...
- Ask AI: an accent "✦ Ask AI" button leads the selection toolbar, and the header has an AI tool. Both open one menu: next to the selected text on a wide screen (under the header tool when nothing is selected), a bottom sheet over a scrim on a phone. Three groups (Edit selection, Write, Whole document); the running command shows the spinner and the menu stays until the answer lands, then closes and selects the new suggestion, which reads "Suggested by AI". In comments, AI is the last name in the @ list and replies as "CoWrite AI".
