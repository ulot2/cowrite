...
- Document header: on the left, "‹ Documents" and the status pill (Draft grey, In review warning, Approved green; owners and reviewers open it for the moves they may make). The title is not repeated there; it is the big heading below. On the right: the presence avatars (with a short pulsing "Connecting" or "Offline" pill when needed), then two outlined groups of icon tools with tooltips (Suggest, Ask Nib, Outline, Comments; then History and Export), then Share as the one filled accent button. A pressed tool turns accent on the accent wash. When the row is too narrow it wraps instead of overlapping. On phones: "‹ Documents" and the pill on top, the tools spread across a second row, Share as an icon.
