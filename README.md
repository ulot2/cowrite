...
Documents without an account: "Try it" (`/play`) opens the browser's newest guest document, or makes a first one with a short tour. A guest is a random id in the HttpOnly `guest` cookie; `/g` lists its documents (10 at most) and `/g/<id>` opens one. A guest document is a normal document room plus a row in `guest_documents`, so the editor, versions, suggestions, offline edits, and export work as for any document. Its room writes only that row to D1, and deletes itself 30 days after its last edit. When the person signs up or signs in, the first page moves the rows into `documents` with the same ids, so the text stays where it is (`app/lib/guest.server.ts`). New guests are limited to 20 a day per IP address, counted as a one-way code.
