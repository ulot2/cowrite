...
export type DocumentRow = { id: string; title: string; preview: string; open_comments: number; updated_at: number; role: Role }
...
export const listDocuments = async (userId: string, { q = '', limit = 500 } = {}) =>
  (await env.DB.prepare(
    `SELECT d.id, d.title, d.preview, d.open_comments, d.updated_at, m.role FROM documents d JOIN memberships m ON m.document_id = d.id
