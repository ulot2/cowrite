...
export type DocumentRow = { id: string; title: string; updated_at: number; role: Role }

// Documents this user can open, newest change first.
export const listDocuments = async (userId: string) =>
  (await env.DB.prepare(
    'SELECT d.id, d.title, d.updated_at, m.role FROM documents d JOIN memberships m ON m.document_id = d.id WHERE m.user_id = ? ORDER BY d.updated_at DESC',
  ).bind(userId).all<DocumentRow>()).results
